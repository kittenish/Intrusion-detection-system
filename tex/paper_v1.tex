...
\begin{itemize}
\item \textbf{Denial of Service Attack (DoS):} This type of attack aims at making the target computer's service unavailable to its legitimate users by increasing the number of significant resource demanding process to exhaust the target's network bandwidth , CPU load, memory and etc\cite{bhuyan2014detecting}. However, with the development of detection and IP tracking techniques, old single source attacks can be easily detected and the source can be rebuffed or shut down. A distributed Dos(DDos) attack attack technique has been developed, which is very difficult to defend. This survey\cite{zargar2013survey} is highly recommended for more information.
\item \textbf{User to Root Attack (U2R)} This type of attack refer to the behavior that the attackers trying to gain root access in the system. Usually the attacker entrance the system as a normal user first and try to exploit some vulnerabilities in the system.
\item \textbf{Remote to Local Attack (R2L):} This type of attacks occurs when an attacker send packets to a remote server where the attacker have no legitimate accounts over a network to gain local access as a legitimate user\cite{mukkamala2005intrusion}. Usually, this illegal accesses are gained by exploiting vulnerabilities of the server by the attackers.
\item \textbf{Probing Attack:} This type of attacks are basically an information collection behavior. The attacker scans the computers in a network to gather information or to exploit vulnerabilities.
For IDSs specially designed for probing attacks, this work \cite{zargar2009identification} is recommended.
\end{itemize}
...
Nearly all the former techniques in supervised learning are used in intrusion detection\cite{ingre2015performance}\cite{sommer2010outside}\cite{qiao2002anomaly}. Both the computational complexity and concrete introduction of the former techniques can be found in \cite{buczak2016survey} and \cite{tsai2009intrusion}.
...
In the approach proposed in this paper, both the supervised learning and ensemble learning methods are employed. The combination of decision tree model and neural networks model demonstrates a great improvement compared with using single model for prediction in NSL-KDD dataset.
...
As shown in Fig \ref{fig:overview}, the decision tree has very simple architecture in the proposed approach.
...
\item The experiments are originally planned to conducted on the UNB ISCX IDS 2012 dataset, which is the up-to-date dataset for intrusion detection. However, it may take months to get the dataset following the procedures on the official website. But still, more experiments should be conducted on this new dataset before making any conclusions.
\end{itemize}
...
\item The datasets that are extensively used currently are still the simulations two decades ago, which makes the models analyzed in these papers unsuitable for detecting newly appeared attacks in recent years. So more datasets are urgently needed in the field of intrusion detection.
\end{itemize}
